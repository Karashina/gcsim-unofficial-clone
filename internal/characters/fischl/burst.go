...
const (
	burstHitmark          = 18
	burstFullOzSpawn      = 113 // アクション開始から
	burstFullOzFirstTick  = 69
	burstShortOzSpawn     = 1 // キャラ交代後
	burstShortOzFirstTick = 63
)

func init() {
	burstFrames = frames.InitAbilSlice(148)
	burstFrames[action.ActionDash] = 115 // シートのダッシュフレームが誤っていたと仮定
...
func (c *char) burstOzSpawn(src, ozSpawn, firstTick int, c4HealFunc func()) func() {
	return func() {
		...
		// 4凸回復はオズ出現/アニメーション終了直後に発生すべき。回復からのバフはオズにスナップショットされない
		if c4HealFunc != nil {
