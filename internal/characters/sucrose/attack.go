...
func init() {
	...
	// TODO: NA->CA と CA->CA のヒットマークが一致するか確認が必要
	attackFrames[0] = frames.InitNormalCancelSlice(attackHitmarks[0], 28) // 歩行
	attackFrames[0][action.ActionAttack] = 17
	attackFrames[0][action.ActionCharge] = 20

	attackFrames[1] = frames.InitNormalCancelSlice(attackHitmarks[1], 38) // 歩行
	attackFrames[1][action.ActionAttack] = 26
	attackFrames[1][action.ActionCharge] = 18

	attackFrames[2] = frames.InitNormalCancelSlice(attackHitmarks[2], 46) // 歩行
	attackFrames[2][action.ActionAttack] = 33
	attackFrames[2][action.ActionCharge] = 28

	attackFrames[3] = frames.InitNormalCancelSlice(attackHitmarks[2], 65) // 歩行
