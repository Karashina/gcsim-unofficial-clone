...
import (
	...
	// servermode package removed. Use a simple http.Handler until a replacement is provided.
	// This preserves cmd/server CLI behavior while removing servermode dependency.
)
...
func main() {
	...
	// Start a minimal HTTP server. The original servermode package provided
	// richer routing and worker management for the web UI. That package has
	// been removed; use a simple handler here to keep the CLI/server command
	// functional without servermode.
	mux := http.NewServeMux()
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintln(w, "gcsim server mode has been removed; this endpoint is a placeholder")
	})

	log.Printf("Starting placeholder server on %s:%s", opt.host, opt.port)
	log.Fatal(http.ListenAndServe(fmt.Sprintf("%v:%v", opt.host, opt.port), mux))
}
